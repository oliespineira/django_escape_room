...
Work from the directory that contains **`manage.py`** and **`requirements.txt`** (if your clone has an extra nested folder with the same name, `cd` into that inner folder).
...
### 4) Environment variables (contributors)

Do this once per machine after cloning:

1. Copy the template (name it `.env` in the **same folder** as `manage.py`):
...
2. Edit `.env` and set `OPENAI_API_KEY=sk-...` **only if** you want these features to work locally:
   - **AI Room Analyst** on a room’s detail page  
   - **AI fleet briefing** on the Analytics page  

   If you leave it blank, the rest of the app works; only those buttons will fail until a key is set.

3. **Never commit `.env`** (it is listed in `.gitignore`). Each contributor uses their own key or skips AI features.

Alternatively, you can skip `.env` and set the variable in the shell before `runserver` (session only):
...
Django loads `.env` automatically via `python-dotenv` when you run the server from the project root.
...
You can **register** a user at `/accounts/register/` or use the superuser from step 7 to sign in.

---

## For contributors (checklist)

After `git pull`, do the following when you open the project on a new machine (or after big dependency changes):

| Step | Action |
|------|--------|
| 1 | `cd` into the repo root (folder with `manage.py`). |
| 2 | Activate the venv (see Quick Start §2). |
| 3 | `pip install -r requirements.txt` |
| 4 | Copy `.env.example` → `.env` and set `OPENAI_API_KEY` if you need AI analytics (optional). |
| 5 | `python manage.py migrate` |
| 6 | Optional: `python manage.py seed_data --clear` for demo data. |
| 7 | `python manage.py runserver` → open `http://127.0.0.1:8000` |

**Do not** commit `.env`, `db.sqlite3`, or `venv/` / `.venv/`. Before opening a PR, run tests: `python manage.py test games`.
